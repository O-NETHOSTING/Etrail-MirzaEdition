...
switch (@$task)
{
	...
	case 'srv-podesavanja':
		...
		{
			if(!ssh2_auth_password($con, $server['username'], $server['password'])) { $error .= "Netačni podatci za prijavu"; }
			else 
			{			
			$cmd1 = 'passwd';
			$cmd2 = $server['password'];
			...
			while($line = fgets($stream)) {
				$data .= $line;
				$pos = strpos($line, "successfully");	
				if($pos !== false) $promenjeno = "da";	
				else $promenjeno = "ne";
			}

			if($promenjeno == "da" OR $password == $server['password'])
			{
	...
	case 'server-add':
		...
		$installdir = mysql_real_escape_string($_POST['installdir']);
		...
		if(query_numrows("SELECT `port` FROM `serveri` WHERE `port` = '".$port."' AND `ip_id` = '{$ipid}'") == 1) $error = "Taj port je vec u upotrebi, javite administratorima za ovu gresku!"; 
		...
		else
		{
			...
			$ssh_dodavanje = ssh_dodaj_server($ipi['ip'], $boxi['sshport'], $boxi['login'], $masina_pw, $username, $sifra, $komanda['putanja'], $installdir);
	...
	case 'updateuser':
		...
		else 
		{
			...
			{	
				...
				$_SESSION['msg-type'] = "success";
				$_SESSION['msg1'] = "Uspešno";
				$_SESSION['msg2'] = "Update ste usera <z>$server[username]</z>";		
				header("Location: srv-pocetna.php?id=".$serverid);
				die();	
			}
		}
	...
	case 'chown':
		...
		else 
		{
			...
			{	
				...
				$_SESSION['msg-type'] = "success";
				$_SESSION['msg1'] = "Uspešno";
				$_SESSION['msg2'] = "Chown usera <z>$server[username]</z>";		
				header("Location: srv-pocetna.php?id=".$serverid);
				die();	
			}
		}
